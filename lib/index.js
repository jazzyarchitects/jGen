...
var initialiseProject = function(file){
...
var __showModels = function(){
  ...
  var name = npmconfig.name;
  var spaceString = "";
  for(var i=name.length;i<len-2;i++){
    spaceString+=" ";
  }
  ...
  fs.readdirSync(modulePath).forEach((dir)=>{
    ...
    var str = "";
    var stat = fs.lstatSync(path.join(modulePath, dir));
    if(stat.isDirectory()){
...
var __tree = function(root){
  ...
  if(typeof(root)==="object"){
    root.forEach((r)=>{
      ...
      var __root = r.split("/").splice(0, r.split("/").length-1).join("/");
      // var __root = r;
